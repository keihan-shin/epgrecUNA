...
#if MoveAribMark
extern char	stock[120];
extern int		treat_mode;
extern int		stock_mode;
#endif
...
static	const DWORD AribToStringInternal(TCHAR *lpszDst, const unsigned BYTE *pSrcData, const DWORD dwSrcLen);
static	const DWORD ProcessCharCode(TCHAR *lpszDst, const unsigned WORD wCode, const CODE_SET CodeSet);

static	const DWORD PutKanjiChar(TCHAR *lpszDst, const unsigned WORD wCode);
static	const DWORD PutAlphanumericChar(TCHAR *lpszDst, const unsigned WORD wCode);
static	const DWORD PutHiraganaChar(TCHAR *lpszDst, const unsigned WORD wCode);
static	const DWORD PutKatakanaChar(TCHAR *lpszDst, const unsigned WORD wCode);
static	const DWORD PutJisKatakanaChar(TCHAR *lpszDst, const unsigned WORD wCode);
static	const DWORD PutSymbolsChar(TCHAR *lpszDst, const unsigned WORD wCode);

static	void ProcessEscapeSeq(const unsigned BYTE byCode);
...
static	const bool DesignationGSET(const BYTE byIndexG, const unsigned BYTE byCode);
static	const bool DesignationDRCS(const BYTE byIndexG, const unsigned BYTE byCode);
...
#if ZenToHan
const DWORD apst_code(TCHAR *lpszDst, DWORD dwDstLen, const unsigned BYTE *pSrcData, DWORD dwSrcPos, const DWORD dwSrcLen, const CODE_SET CodeSet)
{
	const unsigned BYTE bCode = pSrcData[dwSrcPos] & 0x7FU;

	switch( CodeSet ){
		case CODE_ALPHANUMERIC :
		case CODE_PROP_ALPHANUMERIC :
			if( bCode=='!' || bCode=='?' ){
				if( ( dwDstLen>0 && ( lpszDst[dwDstLen-1]=='!' ||  lpszDst[dwDstLen-1]=='?' ) )
						|| ( dwSrcPos+1<dwSrcLen && ( (pSrcData[dwSrcPos+1]&0x7FU)=='!' || (pSrcData[dwSrcPos+1]&0x7FU)=='?' ) ) ){
					lpszDst[dwDstLen] = bCode;
					return 1;
				}else
					return PutAlphanumericChar( &lpszDst[dwDstLen], (const unsigned WORD)bCode );
			}
		default:
			return ProcessCharCode( &lpszDst[dwDstLen], (const unsigned WORD)bCode, CodeSet );
	}
}
#endif
...
const DWORD AribToStringInternal(TCHAR *lpszDst, 
								 const unsigned BYTE *pSrcData, const DWORD dwSrcLen)
{
	if(!pSrcData || !dwSrcLen || !lpszDst)return 0UL;
  
	DWORD dwSrcPos = 0UL;
	DWORD dwDstLen = 0UL;
	unsigned BYTE   dwSrcData;
		...
		if(!m_byEscSeqCount){
			...
			if((dwSrcData >= 0x21U) && (dwSrcData <= 0x7EU)){
				...
				if(abCharSizeTable[CurCodeSet]){
					...
					dwDstLen += ProcessCharCode(&lpszDst[dwDstLen], ((unsigned WORD)pSrcData[dwSrcPos + 0] << 8) | (unsigned WORD)pSrcData[dwSrcPos + 1], CurCodeSet);
					dwSrcPos++;
				}
				else{
					// 1バイトコード
#if ZenToHan
					dwDstLen += apst_code( lpszDst, dwDstLen, pSrcData, dwSrcPos, dwSrcLen, CurCodeSet );
#else
					dwDstLen += ProcessCharCode(&lpszDst[dwDstLen], (unsigned WORD)dwSrcData, CurCodeSet);
#endif
				}
			}
				...
				if(abCharSizeTable[CurCodeSet]){
					...
					dwDstLen += ProcessCharCode(&lpszDst[dwDstLen], ((unsigned WORD)(pSrcData[dwSrcPos + 0] & 0x7FU) << 8) | (unsigned WORD)(pSrcData[dwSrcPos + 1] & 0x7FU), CurCodeSet);
					dwSrcPos++;
				}
				else{
					// 1バイトコード
					...
					dwDstLen += apst_code( lpszDst, dwDstLen, pSrcData, dwSrcPos, dwSrcLen, CurCodeSet );
#else
					dwDstLen += ProcessCharCode(&lpszDst[dwDstLen], (unsigned WORD)(dwSrcData & 0x7FU), CurCodeSet);
#endif
				}
			}
			else{
...
const DWORD ProcessCharCode(TCHAR *lpszDst, const unsigned WORD wCode, const CODE_SET CodeSet)
{
	switch(CodeSet){
	case CODE_KANJI	:
...
#if ZenToHan
		if( wCode>=0x2330U && wCode<=0x2339U ){
			lpszDst[0] = (TCHAR)( wCode & 0x00FFU );
			return 1UL;
		}else
		switch( wCode ){
			case 0x2121U :
...
const DWORD PutKanjiChar(TCHAR *lpszDst, const unsigned WORD wCode)
{
	...
	code[1] = 0x24U;
	code[2] = 0x40U;
	code[3] = (unsigned char)(wCode >> 8);
	code[4] = (unsigned char)(wCode & 0xFFU);
	code[5] = 0x1BU;
	code[6] = 0x28U;
	code[7] = 0x4AU;
	code[8] = '\0';
	...
	memcpy(lpszDst, xcode, strlen(xcode));

	return strlen(xcode);
}

const DWORD PutAlphanumericChar(TCHAR *lpszDst, const unsigned WORD wCode)
{
	// 英数字全角文字コード変換
...
const DWORD PutHiraganaChar(TCHAR *lpszDst, const unsigned WORD wCode)
{
	// ひらがな文字コード変換
...
const DWORD PutKatakanaChar(TCHAR *lpszDst, const unsigned WORD wCode)
{
	// カタカナ英数字文字コード変換
...
const DWORD PutJisKatakanaChar(TCHAR *lpszDst, const unsigned WORD wCode)
{
	// JISカタカナ文字コード変換
...
const DWORD PutSymbolsChar(TCHAR *lpszDst, const unsigned WORD wCode)
{
	...
	static const TCHAR *aszSymbolsTable1[] =
		{
			...
			_T("【声】"),	_T("【吹】"),	_T("【PPV】"),	_T("（秘）"),	_T("【他】")															// 0x7A70 - 0x7A74	90/80 - 90/84
		};
	...
	// シンボルを変換する
#if MoveAribMark
	if((wCode >= 0x7A50U && wCode <= 0x7A72U) || wCode == 0x7A74U ){
		if( treat_mode == OFF )
			strcpy(lpszDst, aszSymbolsTable1[wCode - 0x7A50U]);
		else
			if( stock_mode == ON )
				strcat( stock, aszSymbolsTable1[wCode - 0x7A50U] );
	}
	else if(wCode == 0x7A73U){
		// (秘)
		strcpy(lpszDst, aszSymbolsTable1[0x23]);
	}
#else
...
void ProcessEscapeSeq(const unsigned BYTE byCode)
{
	// エスケープシーケンス処理
	...
	case 1U	:
		...
		case 0x6EU	: LockingShiftGL(2);	m_byEscSeqCount = 0;	return;		// LS2
		case 0x6FU	: LockingShiftGL(3);	m_byEscSeqCount = 0;	return;		// LS3
		case 0x7EU	: LockingShiftGR(1);	m_byEscSeqCount = 0;	return;		// LS1R
		case 0x7DU	: LockingShiftGR(2);	m_byEscSeqCount = 0;	return;		// LS2R
		case 0x7CU	: LockingShiftGR(3);	m_byEscSeqCount = 0;	return;		// LS3R

			// Designation of graphic sets
		case 0x24U	:	
		case 0x28U	: m_byEscSeqIndex = 0;		break;
		case 0x29U	: m_byEscSeqIndex = 1;		break;
		case 0x2AU	: m_byEscSeqIndex = 2;		break;
		case 0x2BU	: m_byEscSeqIndex = 3;		break;
...
const bool DesignationGSET(const BYTE byIndexG, const unsigned BYTE byCode)
{
	// Gのグラフィックセットを割り当てる
	switch(byCode){
	case 0x42U	: m_CodeG[(int)byIndexG] = CODE_KANJI;				return true;	// Kanji
	case 0x4AU	: m_CodeG[(int)byIndexG] = CODE_ALPHANUMERIC;		return true;	// Alphanumeric
	case 0x30U	: m_CodeG[(int)byIndexG] = CODE_HIRAGANA;			return true;	// Hiragana
	case 0x31U	: m_CodeG[(int)byIndexG] = CODE_KATAKANA;			return true;	// Katakana
	case 0x32U	: m_CodeG[(int)byIndexG] = CODE_MOSAIC_A;			return true;	// Mosaic A
	case 0x33U	: m_CodeG[(int)byIndexG] = CODE_MOSAIC_B;			return true;	// Mosaic B
	case 0x34U	: m_CodeG[(int)byIndexG] = CODE_MOSAIC_C;			return true;	// Mosaic C
	case 0x35U	: m_CodeG[(int)byIndexG] = CODE_MOSAIC_D;			return true;	// Mosaic D
	case 0x36U	: m_CodeG[(int)byIndexG] = CODE_PROP_ALPHANUMERIC;	return true;	// Proportional Alphanumeric
	case 0x37U	: m_CodeG[(int)byIndexG] = CODE_PROP_HIRAGANA;		return true;	// Proportional Hiragana
	case 0x38U	: m_CodeG[(int)byIndexG] = CODE_PROP_KATAKANA;		return true;	// Proportional Katakana
	case 0x49U	: m_CodeG[(int)byIndexG] = CODE_JIS_X0201_KATAKANA;	return true;	// JIS X 0201 Katakana
	case 0x39U	: m_CodeG[(int)byIndexG] = CODE_JIS_KANJI_PLANE_1;	return true;	// JIS compatible Kanji Plane 1
	case 0x3AU	: m_CodeG[(int)byIndexG] = CODE_JIS_KANJI_PLANE_2;	return true;	// JIS compatible Kanji Plane 2
	case 0x3BU	: m_CodeG[(int)byIndexG] = CODE_ADDITIONAL_SYMBOLS;	return true;	// Additional symbols
	default		: return false;		// 不明なグラフィックセット
	}
}

const bool DesignationDRCS(const BYTE byIndexG, const unsigned BYTE byCode)
{
	// DRCSのグラフィックセットを割り当てる
	switch(byCode){
	case 0x40U	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-0
	case 0x41U	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-1
	case 0x42U	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-2
	case 0x43U	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-3
	case 0x44U	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-4
	case 0x45U	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-5
	case 0x46U	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-6
	case 0x47U	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-7
	case 0x48U	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-8
	case 0x49U	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-9
	case 0x4AU	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-10
	case 0x4BU	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-11
	case 0x4CU	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-12
	case 0x4DU	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-13
	case 0x4EU	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-14
	case 0x4FU	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// DRCS-15
	case 0x70U	: m_CodeG[(int)byIndexG] = CODE_UNKNOWN;				return true;	// Macro
	default		: return false;		// 不明なグラフィックセット
}
}
